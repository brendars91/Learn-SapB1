...
export const L0 = [
  ...
  sk(0, 1, {
    ...
    c: { es: 'Todo socio tiene una ficha única (código, fiscal, condiciones de pago, cuenta de mayor asociada). Un socio puede ser cliente y proveedor a la vez; el control de duplicados por NIF evita partir su historial en dos. La cuenta contable asociada decide dónde caen sus saldos.', en: 'Every partner has one card (code, tax data, payment terms, linked G/L account). A partner can be customer and vendor simultaneously; duplicate control by tax ID prevents splitting history. The linked account decides where balances land.', de: 'Jeder Geschäftspartner hat eine Karte (Code, Steuerdaten, Zahlungsbedingungen, verknüpftes Sachkonto). Ein Partner kann gleichzeitig Kunde und Lieferant sein; die Dublettenkontrolle per Steuer-ID verhindert geteilte Historien. Das verknüpfte Konto bestimmt, wohin die Salden fallen.' },
