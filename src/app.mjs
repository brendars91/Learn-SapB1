...
function svgRadar(record, state, level = 0) {
  ...
  const ring = f => `<polygon points="${dims.map((_, i) => pt(i, R * f).join(',')).join(' ')}" fill="none" stroke="var(--rule-strong)" stroke-width="${f === 1 ? 1.2 : 0.7}"/>`;
  const rings = [0.33, 0.66, 1].map(ring).join('');
  const spokes = dims.map((_, i) => { const [x, y] = pt(i, R); return `<line x1="${cx}" y1="${cy}" x2="${x}" y2="${y}" stroke="var(--rule-strong)" stroke-width="0.7"/>`; }).join('');
  const vals = dims.map(([f]) => Math.max(0, Math.min(100, Number(record?.[f]) || 0)) / 100);
  const poly = `<polygon points="${vals.map((v, i) => pt(i, R * v).join(',')).join(' ')}" fill="rgba(139,44,44,0.16)" stroke="var(--accent)" stroke-width="1.6" stroke-linejoin="round"/>`;
  const apexes = vals.map((v, i) => v > 0 ? pt(i, R * v) : null).filter(Boolean)
    .map(([x, y]) => `<circle cx="${x}" cy="${y}" r="2.4" fill="var(--gold)" stroke="var(--accent)" stroke-width="1"/>`).join('');
  const labels = dims.map(([f, key], i) => {
    const lateral = i === 1 || i === 3;
    const [x, y] = pt(i, R + (lateral ? 27 : 20));
    const anchor = i === 1 ? 'end' : i === 3 ? 'start' : 'middle';
    const dy = i === 0 ? -2 : i === 2 ? 8 : 3;
    return `<text x="${x}" y="${y + dy}" text-anchor="${anchor}" font-size="9.5" letter-spacing="0.04em" fill="var(--ink-soft)">${escapeHtml(translate(state.locale, key))}</text>`;
  }).join('');
  return `<svg class="sbl-radar" viewBox="0 0 232 192" role="img" aria-label="${t(state, 'radarLabel')}">${rings}${spokes}${poly}${apexes}${labels}</svg>`;
}
